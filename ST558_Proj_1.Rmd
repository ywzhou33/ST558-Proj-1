...
weather <- Full.data %>% 
        mutate(area = if_else((lon>=-120.74)&(lon<=-75.59), "Land", 
               ...
               temp_day = (min+max)/2) %>% # calculate the average  daily temperature.
        as_tibble()  
```
...
knitr::kable(table(weather$area, weather$wind_status))
```
...
Below is a boxplot that shows the distribution of the average daily temperature for the land and water/ocean area around the globe at latitude of 35.78. It confirms the trends displayed in the histogram that the distribution of average temperature is highly skewed and bi-model for the land area and the water area tend to have higher average daily temperature in the future 7 days.
...
coord_flip() 
```
From the barchart below, I found that the wind status on the land area is more calm while it is more windy over the water area aross the globe at latitude 35.78 for the future 7 days.  
...
bar_wind + geom_bar(aes(fill = as.factor(area)), position = "dodge") + 
  ...
  scale_fill_discrete(name = "Area")
```
This boxplot indicates that the humidity over the water areas is high in general that that on the land areas and the humidity varies more on the land areas.  
...
```{r scatter, echo=TRUE}
correlation <- cor(weather$wind_speed, weather$temp_day)
correlation
